...
def mock_component_manager():
    ...
    cli_support.files = [{"src": "cli.py.j2", "dest": "src/{{cookiecutter.package_name}}/cli.py"}]
    ...
    manager.components = {"python_core": python_core, "cli_support": cli_support, "docker": docker}
...
@patch("tempfile.mkdtemp")
def test_compose_template_file_operations_error(
    mock_mkdtemp, template_composer, sample_template_config, mock_component_manager
):
    """Test template composition when file operations fail."""
    mock_mkdtemp.return_value = "/tmp/test"
    mock_component_manager.resolve_dependencies.return_value = ["python_core"]

    with patch("pathlib.Path.mkdir", side_effect=OSError("Permission denied")), pytest.raises(OSError):
...
def test_merge_component_files_missing_source(template_composer, mock_component_manager):
    ...
    with (
        patch("shutil.copy2", side_effect=FileNotFoundError("Source file not found")),
        patch("pathlib.Path.mkdir"),
